...
| Run cache (§19.2): stored traces reused instead of repetitions paid for again, keyed on payload/scenario content/target/fixture/harness/model id/image/platform baseline plus the repetition index, pinned sampling and companion digests; `--no-cache`, `execution.cache_ttl_days`; replays re-filed with `cached_from` provenance, counted in the summary and excluded from the spend the budget gates bound; unpinned claude-code targets bypass it, disclosed | done |
| Pre-flight estimate (§19.1): matrix size, best/expected/worst run counts, token cap, cost range where priced; printed before anything is spent; `--yes` skips the prompt, never the estimate | done |
