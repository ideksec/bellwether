...
Every `run` prints a pre-flight estimate first (§19.1) — run counts and, where every target alias
has `providers.<name>.pricing`, a cost ceiling — and asks to proceed on a terminal (`--yes` skips
the prompt). Repetitions whose key matches a stored trace are served from `<out>/.cache/runs`
instead of being paid for again (§19.2; `--no-cache` to force execution), and the summary says
...
| Per-run limits (§9.2, §12.7): `execution.limits` bounds turns, tool calls and tokens (`--max-tokens` overrides the cap); the wall clock stays the scenario's (§7.2); every run header records the bounds it ran under, `doctor` states them before anything is spent, and raising a ceiling misses the run cache | done |
| Run cache (§19.2): stored traces reused instead of repetitions paid for again, keyed on payload/scenario content/target/fixture/harness/model id/image/platform baseline plus the repetition index, pinned sampling, companion digests and an observability fingerprint (wiring a plane misses, so a replay never reports an unwatched plane as watched); `--no-cache`, `execution.cache_ttl_days`; replays re-filed with `cached_from` provenance, counted in the summary and excluded from the spend the budget gates bound; operator-limit outcomes and unpinned claude-code targets never cached | done |
| Pre-flight estimate (§19.1): matrix size, best/expected/worst run counts, token cap, cost range where priced (the ceiling prices the token cap at each target's dearest rate, so it bounds the spend); printed before anything is spent; `--yes` skips the prompt, never the estimate | done — a decline exits 4, not the infrastructure code |
| `--deterministic-sampling` (§9.3, §20): temperature pinned on every api-loop request, recorded on the header and marked in the report; refused on claude-code | done |
| §7.4 companions on `claude-code`: a scenario's `also_load_skills` are staged beside the skill under test for the CLI to discover (observed in its init record as `skill_offered`); collisions refused before any copy; the api-loop path unchanged | done — plugin-layout staging still open |
