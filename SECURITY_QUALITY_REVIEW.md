...
| BW-51 | Medium | LIVE | The §16.4 precondition check is built, exported and unit-tested but has **no caller in `src/`** — an unsatisfiable policy is discovered only after a full matrix is paid for (fails safe; disclosed, not fixed) |
...
- **Disposition: disclosed, not fixed in this pass.** Wiring it needs each target's `TargetDeclaration` (harness capabilities) and the runner's available-plane set threaded into `run`; getting those wrong would make the check *refuse valid runs*, which is a worse failure than the current one. Deliberately not attempted at the end of a release-hygiene pass. Tracked in `docs/STATUS.md` → Outstanding actions, and `doctor` now states plainly that the check "is built and tested but not yet wired into doctor or run" rather than attributing its absence to a completed work package.
- **Claim corrected:** README's build table and STATUS both read "verdict engine + precondition check — done". The *engine* is done; the *wiring* is not. Both now say so.
