...
| BW-47† | Medium | LIVE | **Fixed in the public-release pass:** declared manifest scope now applies on the live `run` path (was `scope=None`) — see the pass section below |
| BW-49 | Medium | LIVE | Only `egress_outside_allowlist` drives the scored verdict; the other 12 `security_runtime` dispositions are captured but never gate — `doctor` names them (disclosure); **first slice closed in a follow-up brick**: `canary_leak` is now a scored gate (`security_runtime.canaries`) — a leaking skill can no longer reach `ready` |
| BW-50 | Low | LIVE | Egress host/SNI canary scan missed a subdomain-tunnel exfil on a case fold (host recorded lowercased, marker mixed-case) — **fixed** |
| BW-51 | Medium | LIVE | The §16.4 precondition check is built, exported and unit-tested but has **no caller in `src/`** — an unsatisfiable policy is discovered only after a full matrix is paid for (fails safe) — **fixed in the follow-up brick**: `run` refuses before the executor is built, `doctor` evaluates per profile (`cli/preflight.py`) |
| BW-52 | Low | LIVE | Doc/example drift found by the public-release read: the live example claimed the proxy "is not yet wired" while its own config turns it on; `ci-integration.md` said the live CI run was unproven while the README called it proven; `doctor` attributed missing probes to *completed* work packages — **all fixed** |
...
- **BW-47** — was **deferred** at this pass (still `scope=None`), so the scope gate rendered a green `pass / within scope` for a skill that used a manifest-denied tool. **Superseded: fixed in the public-release review pass below** — declared scope is now applied on the live `run` path, decoupled from the still-stubbed network/write derivations.

---

## Public-release review pass (pre-v0.1)

An independent, adversarial pass ahead of making the repository public, run the same way — four parallel subsystem reviews plus a hands-on pass, every reported issue reproduced by running code, hunting specifically for the project's signature failure mode: **a broken observation/control path that produces a reassuring-looking clean result.** The crown-jewel invariants held (the sandbox has no observer inside it; the trace is deterministic and redacted; the recording proxy isolates the real key; every BW-01…BW-48 fix above still holds). The pass found that the *live* verdict path under-enforced relative to what the demo and the policy imply, and closed the two highest-value gaps with regression tests. **Baseline now: six checks green, 834 offline tests pass, 45 docker pass / 6 CI-only skip, `PYTHONHASHSEED=12345` byte-stable, `bellwether demo` regenerates with no diff.**

The stance for release is deliberate: **fix the reachable false-greens, and disclose — do not conceal — the enforcement that is still roadmap breadth.** The disclosures live in `README.md` ("What the live verdict gates today"), `THREAT_MODEL.md`, and `bellwether doctor`, so a user learns the boundary from the tool, not from this file.

### BW-47 (was deferred) — FIXED: declared manifest scope now applies on the live `run` path
- **Where:** `cli/run.py` passed `scope=None` into `drive_evaluation` and nothing else; `cli/orchestrator.py:_scope_result` rendered a disposition from an always-empty `scope_exceeded`.
- **Status:** **CONFIRMED pre-fix, then closed.** A transcript that activates the skill and then calls a tool the manifest does not declare reached the `scope` gate as `pass / within scope` — a clean-looking verdict with the check never run.
- **Why it matters:** this is the exact target bug class. A skill could use a manifest-denied capability and still reach `ready`, while the demo (which *does* apply scope, via a separate path) implied the control was live everywhere.
- **Fix:** thread the manifest's `declared_scope` through `drive_evaluation` as a declared-vs-observed table (`scope_exceeded_of`), evaluated off the run outcome so the still-stubbed network/write derivations (§10.5) do not drag a clean run to `not_evaluable` — the same split the demo already used. Undeclared dimensions stay allow-all (`_tool_rows` returns `[]` on an empty allow-list), so a skill that declares nothing is never falsely blocked. See spec-notes §12.5, §16.2.
- **Regression:** `test_drive_evaluation_enforces_declared_scope_on_the_live_path` — differential: `scope_exceeded=("read",)` with the declared scope threaded in, `()` without; fails `() == ('read',)` against the pre-fix driver.
- **Residual (disclosed, not fixed):** the network/write scope *derivations* remain stubbed to `not_evaluable`, so an *undeclared-egress* violation is not yet scored here — only the declared tool/filesystem-read table is. That is the network plane's roadmap breadth, now stated rather than papered over.

### BW-49 — Only `egress_outside_allowlist` drives the scored verdict; the other runtime dispositions are inert (Medium, LIVE)
- **Where:** `cli/orchestrator.py` assembles exactly five gates; `security_runtime` contributes only `security_runtime.egress`. `SecurityRuntimeGate` carries 13 disposition fields and the scaffold policy sets most to `block` (`canary_leak`, `dns_outside_allowlist`, `credential_read_undeclared`, `sensitive_directory_access`, …).
- **Status:** **CONFIRMED by repro.** A grep of every disposition field's readers plus the gate assembly shows canary/DNS/credential/directory/probe/anomaly findings are captured and reported but never reach a gate; a `canary_leak: block` policy does not, on its own, make a leaking run `not_ready`.
- **Why it matters:** a configured `block` reads as an active control while it does nothing — the same silent-no-op trap already surfaced for `require_scan` (BW-19). This is capture-without-enforcement, not a false artifact, but it is easy to mistake for a gate.
- **Fix (legibility, not enforcement):** `ENFORCED_SECURITY_RUNTIME_DISPOSITIONS` next to the gate assembly is the one authoritative list of what gates; `doctor` reads it to warn, naming exactly which configured dispositions are inert. Wiring each into a gate is per-plane roadmap work (WP-16 live canaries, WP-15 DNS scoring, credential/scope planes). `dns_outside_allowlist` still gates *runnability* in the §16.4 precondition; it is listed because it is not *scored*.
- **Regression:** `test_doctor_warns_that_most_runtime_dispositions_do_not_gate_yet`.
- **First slice CLOSED in a follow-up brick: `canary_leak` is now a scored gate** (`security_runtime.canaries`, from Plane C findings). The flagship differential: the exact evidence that previously yielded `conditional` — six passing runs, one with a `canary_leak` finding — now yields `not_ready` under the shipped `canary_leak: block` (`test_a_canary_leak_blocks_the_verdict`, verified to fail with the gate assembly neutralized: `'conditional' == 'not_ready'`). Unplanted runs defer as `not_evaluable`; an observed-clean set passes even at the live path's `partial` fidelity, because the partial gap (the model-API channel) feeds `canary_without_read` — a different finding class whose destinations are not part of the leak scan — while every leak-class destination is scanned whenever the plane exists. `canary_without_read` stays deliberately unscored (its evidence cannot exist until model-channel read-state scanning lands — scoring it now would recreate this very finding), and `doctor`'s enforced-vs-inert lists now both derive from the one constant so the message cannot drift. The §16.4 preflight gained the matching clause: `canary_leak: block` with canaries disabled refuses before spending. Remaining inert and disclosed: `dns_outside_allowlist`, `credential_read_undeclared`, `canary_without_read`, `sensitive_directory_access`, and the anomaly checks.

### BW-50 — Egress host/SNI canary scan missed a subdomain-tunnel exfil on a case fold (Low, LIVE)
- **Where:** `trace/build.py:_scan_source_action` joined path+host+SNI and scanned case-sensitively; a canary marker is mixed-case while `_norm_host` records the host lowercased (`urlsplit().hostname`).
- **Status:** **CONFIRMED pre-fix, then closed.** A marker tunnelled through a subdomain (`<marker>.attacker.com`) is recorded lowercased and the case-sensitive scan misses it — the URL scan reported nothing while the secret rode out. The prior test masked this by building the `EgressFlow` with a raw, un-normalised host, so it never exercised the lowercasing.
- **Fix:** scan the path case-sensitively and decode-aware (a path can carry a case-exact base64 marker), and the host/SNI the way a DNS name is scanned — case-folded, label-split, base32-aware; de-duplicated to one finding per request. Path-marker offsets are unchanged. See spec-notes §10.4.2.
- **Regression:** `test_a_marker_tunnelled_through_a_lowercased_subdomain_is_found` (records the host through `_norm_host`; fails `[] == ['canary_leak']` pre-fix) and `test_a_canary_in_both_path_and_host_is_reported_once`.

### BW-51 — The §16.4 precondition check is built, tested, and never called (Medium, LIVE)
- **Where:** `verdict/precondition.py:67` (`check_preconditions`), exported via `verdict/__init__.py` and covered by `tests/test_verdict.py`.
- **Status:** **CONFIRMED by repro.** A grep for callers across the whole package returns the definition and the export and nothing else — no call site in `cli/run.py`, `cli/app.py` (`doctor`), or the orchestrator. The check never executes in any shipped path.
- **Why it matters:** §16.4 says the orchestrator MUST compare policy requirements against target capabilities and runner plane coverage and **refuse to start** if they cannot be satisfied, and §20 says surface the same check in `doctor` "so a user learns before a forty-minute run rather than after it." Neither happens: an unsatisfiable profile (e.g. `high`, which requires the process plane that does not exist) is discovered only after a full matrix has been paid for. This is the project's signature pattern — built, unit-tested, never wired — and the same one as BW-47.
- **Direction of failure:** *safe.* The gates still block correctly on `not_evaluable`, so no false green is produced; the cost is wasted spend and a violated spec MUST, not an unearned verdict. That is why it is Medium rather than High.
- **Disposition at review time: disclosed, not fixed.** Wiring it needs each target's `TargetDeclaration` (harness capabilities) and the runner's available-plane set threaded into `run`; getting those wrong would make the check *refuse valid runs*, which is a worse failure than the current one. Deliberately not attempted at the end of a release-hygiene pass.
- **FIXED in the follow-up brick** (`cli/preflight.py`). The wiring is composition-derived, which is what the refuse-valid-runs concern demanded: a plane is available exactly when config wires its component (`egress.image` → proxy, `dns.image` → resolver, `canaries.enabled` → credentials; the read/process planes are never available in this version), and each target's declaration is the adapter's static capabilities overlaid with those bits — so the proven live configuration passes and the scaffold default (blocking egress gate, no proxy) refuses with the remedy named. En route the check's bundled egress/DNS clause was split per channel (the two components are wired independently, so a wired proxy must not vouch for an unwired resolver), and a target naming a harness with no shipped adapter now refuses up front — previously it ran the whole sandbox under the api-loop adapter and died on the trace-to-plan binding, money spent, wrong error. `doctor` evaluates the check per profile instead of listing it pending. Regressions: three run-level refusal tests (each verified to fail with the preflight call neutralized — the no-adapter one fails by *building the executor*, proving pre-fix spend), a per-channel unit split in `test_verdict.py`, and a doctor row test. Residual, disclosed: `requires.min_bellwether_version` is not checked — version ordering needs a rule the project has not committed to, and the one profile that sets it already refuses on its missing planes, so the skip cannot produce a false start today.

### Noted, low-reachability (folded into existing disclosures, not new findings)
- **Provider-subdomain scan exemption.** The URL/body canary scan exempts `model_api`-class requests, and classification is suffix-matched, so in principle a provider *subdomain* would inherit the exemption. Reachability is bounded by the default-deny egress allowlist and the fact that an attacker does not control provider subdomains, so this is a facet of the already-disclosed **residual model-API channel** (README, THREAT_MODEL), not an independent hole. Left as disclosure.
